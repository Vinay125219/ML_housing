...
class MLOpsMetrics:
    ...
    def record_prediction(self, model_type: str, model_name: str, 
                         prediction_value: float, latency: float):
    ...
    def update_model_performance(self, model_type: str, model_name: str, 
                                metrics: Dict):
    ...
    def update_daily_predictions(self):
        ...
        try:
            # Housing predictions
            housing_count = self._get_daily_prediction_count('housinglogs/predictions.db', 'housinglogs')
            self.daily_predictions.labels(model_type='housing').set(housing_count)
            
            # Iris predictions
            iris_count = self._get_daily_prediction_count('irislogs/predictions.db', 'irislogs')
    ...
    def update_retraining_status(self, status_info: Dict):
        ...
        try:
            ...
            if 'housing' in status_info and 'last_retrain' in status_info['housing']:
                timestamp = datetime.fromisoformat(status_info['housing']['last_retrain']).timestamp()
            ...
            if 'iris' in status_info and 'last_retrain' in status_info['iris']:
                timestamp = datetime.fromisoformat(status_info['iris']['last_retrain']).timestamp()
